...
1. **Python Syntax Check**
   ...
   - Command: `cd backend && uv run python -m py_compile main.py auth.py health.py schemas.py`
...
2. **All Backend Tests**
   - Preparation Command: None
   - Command: `cd backend && uv run pytest tests/ -v --tb=short`
   - test_name: "all_backend_tests"
   - test_purpose: "Validates all backend functionality including OCR processing, LLM integration, document processing pipelines, and API endpoints"

### Frontend Tests

3. **Frontend Linting**
   - Preparation Command: None
   - Command: `cd frontend && npx @biomejs/biome check .`
   - test_name: "frontend_linting"
   - test_purpose: "Validates frontend code quality using Biome, identifies unused imports, style violations, and potential bugs"
...
[
  ...
  },
  {
    "test_name": "all_backend_tests",
    "passed": true,
    "execution_command": "cd backend && uv run pytest tests/ -v --tb=short",
    "test_purpose": "Validates all backend functionality including OCR processing, LLM integration, document processing pipelines, and API endpoints"
  }
]
```
