...
- If you need a new Python library, use `cd backend && uv add <pkg>`. If you need a new frontend library, use `cd frontend && npm install <pkg>`. Be sure to report it in the `Notes` section of the `Plan Format`.
...
- `CLAUDE.md` - Contains the project overview, tech stack, and constraints.
- `backend/main.py` - FastAPI application entry point with lifespan, middleware, and route registration.
- `backend/routers/process.py` - Processing API endpoints (upload, OCR, LLM).
- `backend/routers/models.py` - Model listing/selection endpoints.
- `backend/processors/**` - Document processing pipelines (scan conversion, classification, comparison, handwriting removal).
- `backend/services/**` - Core services (OCR, LLM, formatters).
- `backend/schemas.py` - Pydantic request/response models.
- `backend/auth.py` - Supabase JWT authentication middleware.
- `backend/models/**` - OCR block data models.
- `backend/utils/**` - File validation utilities.
- `backend/tests/**` - Backend test suite (pytest).
- `frontend/app/**` - Next.js App Router pages (dashboard, login, process).
- `frontend/components/**` - React components (FileUpload, ModelSelector, etc.).
- `frontend/lib/**` - API client, types, Supabase client/server helpers.
...
- `cd backend && uv run pytest -x -q` - Run backend tests to validate the feature works with zero regressions
- `cd frontend && npx @biomejs/biome check .` - Run frontend linting
- `cd frontend && npx tsc --noEmit` - Run TypeScript type check
- `cd frontend && npm run build` - Run frontend build to validate no build errors
